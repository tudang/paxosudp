...
EXEC_DIR="/home/danghu/paxosudp/centos-build/sample"
...
do
  ...
  ssh node$c "nohup $EXEC_DIR/learner $CONFIG_FILE > experiment_log/learner$i.log 2>&1 &"
 # ssh node$c "nohup bwm-ng $EXEC_DIR/bwm-ng.conf > $BWM_DIR/$N/learner$i.csv 2>&1 &"
done

#run the client
$EXEC_DIR/client $CONFIG_FILE 0 1 8192 1 
